...
static void hands_update_proc(Layer *layer, GContext *ctx) {
  ...
  PBL_IF_ROUND_ELSE((graphics_draw_circle (ctx, GPoint(90, 90), 3)), (graphics_fill_circle (ctx, GPoint(72, 84), 3)));
}
...
void handle_appmessage_receive(DictionaryIterator *received, void *context) {
  ...
  while (tuple) {
    switch (tuple->key) {
        case KEY_BACK_COLOR:
        key_back_color = tuple->value->int32;
        break;
   }
...
static void window_load(Window *window) {
  ...
  bitmap_layer_set_background_color(s_background_layer, GColorKEY_BACK_COLOR);
